...
public class MinFallingPathSum {
    ...
    public int minFallingPathSum(int[][] A) {
        ...
        for(int i=0;i<A[0].length; i++){
            max = recure(A, 0, i, 0, max);
        }

        return max;
    }

    public int recure(int[][] A, int x, int y, int sum, int max){
        if(x>=A.length || x<0 || y>=A[x].length || y<0) {
            return max;
        }
        sum += A[x][y];
        if(x == A.length-1){
        ...
        max = recure(A, x+1, y-1, sum, max);

        max = recure(A, x+1, y, sum, max);
